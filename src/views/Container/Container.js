...
class Container extends Component {
  ...
  async componentDidMount(){
    await axios.get('http://localhost:3000/entities/container?page=1&limit=10').then( response  =>  {
        if(response){
...
// Delete Entities
   ...
   async deleteItem(entityid){
    if (window.confirm("Delete the item?")) {
        ...
        await axios.delete('http://localhost:3000/entities/container/'+entityid).then( response  =>  {
          if(response){
          }
        ...
        await axios.get('http://localhost:3000/entities/container?page=1&limit=10').then( response  =>  {
                ...
                if (this.state.currentPosts.length <= this.state.postsPerPage){
                  axios.get('http://localhost:3000/entities/container?page=1&limit=10').then(  async response  =>     {
                  if(response){
...
// EDIT Entities
  ...
  async saveChanges (entityid,barcode,batchsize) {
     ...
     await axios.put('http://localhost:3000/entities/container/'+entityid,updateData).then( response  =>  {if (response.data!="OK") {window.alert(response.data);} else {/* window.confirm(response.json().data); */}
    })
